...
export default async function Dashboard({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <div className="np-shell">
    ...
    <main className="np-main">
      ...
      {view === "profiles" ? <><PageTitle title="Profiles" copy="Manage your Amazon, Walmart, and Pokémon Center preferences." /><div className="np-profile-grid">{retailers.map(retailer => <RetailerForm key={retailer.id} retailer={retailer} profile={profilesByRetailer.get(retailer.id)} />)}</div></> : null}
...
function RetailerForm({ retailer, profile }: { retailer: typeof retailers[number]; profile?: ServiceProfile }) {
  return <article className="retailer-card"><div className="retailer-head"><div><span>{retailer.eyebrow}</span><h3>{retailer.name}</h3></div><span className={`profile-badge ${profile ? "ready" : ""}`}>{profile ? profile.status : "Not set up"}</span></div><p>{retailer.description}</p><details><summary>{profile ? "Edit preferences" : "Set up profile"}</summary><form action={saveServiceProfile}><input type="hidden" name="retailer" value={retailer.id} /><label>Service type<select name="serviceType" defaultValue={profile?.service_type ?? "checkout"}><option value="checkout">Checkout assistance</option>{retailer.id !== "pokemon-center" ? <option value="account-generation">Account generation</option> : null}<option value="both">Both services</option></select></label><label>Retailer account email<input name="accountEmail" type="email" autoComplete="email" defaultValue={profile?.account_email ?? ""} placeholder="name@example.com" /></label><label>Target quantity<input name="targetQuantity" type="number" min="1" max="100" defaultValue={profile?.target_quantity ?? 1} required /></label><label>Product preferences<textarea name="productPreferences" maxLength={500} defaultValue={profile?.product_preferences ?? ""} placeholder="Products, sizes, colours, or acceptable alternatives" /></label><label>Additional notes<textarea name="notes" maxLength={1000} defaultValue={profile?.notes ?? ""} placeholder="Optional instructions—never include passwords or payment details" /></label><button className="save-button" type="submit">{profile ? "Update profile" : "Save profile"}</button>{profile ? <small>Updated {new Date(profile.updated_at).toLocaleDateString("en-CA")}</small> : null}</form></details></article>;
}
